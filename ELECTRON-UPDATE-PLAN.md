...
### Follow-up: the keyword-hash fix reworked, plus a rendering regression and the real InstaRepl answer

Continued use surfaced three more things. The hash diagnosis above was right, but the fix was incomplete and the approach has been reworked.

**1. The hash theory, now proven numerically.** Taking the `:clojure.lang` keyword out of a live plugin-created object and comparing it against a freshly-built one: both print `:clojure.lang` and are `=`, but the stale one carries a baked `_hash` field of `4535056938` and hashes to `240089642`, while the fresh one hashes to `-1686276330`. Looking `:clojure.lang` up in `@tags` returns `nil` with the stale keyword and the correct behavior list with the fresh one.

**2. The fix now re-interns keywords instead of string-keying maps.** String keys fixed `@behaviors` and `:listeners`, but could not fix object **tag sets** — `(:tags obj)` is a set of keywords that gets invoked as a function (`(ts tag)`) and read by plugins, so it has to stay keywords. That gap left every *language* object broken: `clj_lang`, `python-lang` and `nodejs-lang` each had only `:destroy` wired, because their `:tags #{:clojure.lang}` came from compiled plugin code. So `on-eval.one` fired correctly and then raised `:eval!` into the void. Replaced the string-keying with `lt.object/fresh-kw`, which re-interns a keyword (dropping the stale cached hash; `cljs.core/keyword` computes lazily with current code), applied at every point a keyword crosses from plugin code into the object system: behavior names and `:triggers`, object `:tags` in `make-object*`/`create`, and the tag arguments of `add-tags`/`remove-tags`/`by-tag`/`has-tag?`/`in-tag?`/`tag-behaviors`. Internal maps stay idiomatically keyword-keyed, and `tag-behaviors`' public signature is unchanged. One trap worth remembering: some templates declare `:tags` as a **vector**, so the normalizer must preserve collection type and `create` must still coerce to a set — an early version returned a lazy seq and the whole app failed to boot with `c.call is not a function` from `by-tag`.

Result, traced live end to end: `on-eval.one` → `eval!` → `try-connect` → `check-all` → `check-java`/`check-ltjar`/`find-project` → `notify` → `run-jar` → `proc/exec java -Xmx1g -jar lein-light-standalone.jar LightTable-REPL`.

**3. A real rendering regression from Phase 2a.** Phase 2a recorded `overlay.js` as "a verbatim copy of CodeMirror 4.1.1's `addon/mode/overlay.js`" and repointed `editor.cljs` at upstream. That was wrong. LightTable's fork passes the base mode's token as a **third argument**: `overlay.token(stream, state.overlay, {pos, style})`; upstream passes two. The Rainbow plugin's token fn reads `base.style` and `base.pos`, so against upstream `base` is `undefined` and *every* syntax-highlight pass throws, aborting CodeMirror line rendering — the editor stops visibly updating as you type. This only became visible once fix #2 above actually wired `rainbow-parens` up. Restored as `deploy/core/lighttable/codemirror/overlay.js`: current upstream CM5 plus that one fenced deviation, so CM5's own fixes are kept. README corrected.

**4. InstaRepl is a separate plugin that was never installed.** The Clojure plugin's CHANGELOG, 0.3.0: *"Split out instarepl into its own plugin - ClojureInstarepl."* `script/build.sh` clones Clojure, CSS, HTML, Javascript, Paredit, Python and Rainbow — not `ClojureInstarepl` (which exists upstream at tag `0.3.0`). So no amount of core fixing would have produced an InstaRepl; the feature simply isn't present. The `:editor.clj.instarepl` tag still referenced in `rainbow.behaviors` is a vestigial hook.

**Remaining blocker for Clojure eval (environmental, not code): the bundled nREPL runner requires Java 8.** `lein-light-standalone.jar` embeds Leiningen 2.5.2, whose `dynapath` references `sun.misc.Launcher$ExtClassLoader` — removed in Java 9. Verified failing identically on JDK 11, 17 and 21 with `ExceptionInInitializerError` → `ClassNotFoundException: sun.misc.Launcher$ExtClassLoader`. No JVM flag can restore a class that no longer exists. Either install a Java 8 JDK and point the plugin at it (the plugin exposes "Clojure: set the path to the Java executable for clients" as a user behavior), or rebuild the runner jar against a modern Leiningen. Note this applies to Linux too — JDK 11 there will fail the same way.
